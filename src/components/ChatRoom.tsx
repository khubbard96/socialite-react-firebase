...
const ChatMessage: React.FC<{ sender: string; text: string }> = ({
  sender,
  text
}) => {
  return (
...
const SendChat: React.FC = () => {
  ...
  const sendMessage = (e: any) => {
    ...
    if (message) {
      groupRef
        .collection("messages")
        .add({ sender: me.displayName, text: message, createdAt: Date.now() });
    }
    setMessage("");
  };
...
const ThreadSelector: React.FC<{ id: string }> = ({ id, children }) => {
  ...
  return <button onClick={changeThread}>{children}</button>;
};
